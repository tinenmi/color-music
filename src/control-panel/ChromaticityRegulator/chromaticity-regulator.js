...
const mobileOrTablet =  typeof(window) == 'undefined' || (()  => {
  var check = false;
...
class SaturationValueRegulator extends PureComponent {
  ...
  onTap = (e) => {
    ...
    if (mobileOrTablet && pointer.height == 1)
      return;
    let saturation = relative(0, pointer.clientX, getWidth(pointer.target))
    saturation = Math.max(0, Math.min(1, saturation))
    let value = 1 - relative(0, pointer.clientY - TOP_OFFSET, getHeight(pointer.target))
    value = Math.max(0, Math.min(1, value))
    setValue({newValue: { saturation, value }})
  }
